...
export function drawSiteMarker() {
  ...
  if (window.siteMarkerPosition && window.siteMarkerPosition.isDragged) {
    ...
    // ⭐ ALWAYS draw indicators if dragged
    // Draw orange circle at original position
    ctx.fillStyle = "#48ff00ff";
    ctx.strokeStyle = "white";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(siteCoords.x, siteCoords.y, 6, 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();
...
/**
 * Draw individual POIs with collision avoidance (NO FALLBACK ICONS)
 */
  ...
  for (const [category, pois] of Object.entries(allPOIsDataByCategory)) {
    for (let idx = 0; idx < pois.length; idx++) {
      ...
      if (
        ...
        // ✅ FIX: Only check collision if NOT dragged
        if (!poi.isDragged) {
          const collision = checkCollisionWithHighways(
            poi.pixelX,
            poi.pixelY,
            poi.logoSize / 2
          );
          ...
          // ✅ If dragged, just draw at dragged position (no collision check, no white lines)
          drawPOILogoStaticCached(poi);
        }
...
/**
 * Draw POI logo with caching (NO FALLBACK - only draw if logo exists)
 */
export function drawPOILogoStaticCached(poi) {
  ...
  if (poi.isDragged) {
    ...
    // ⭐ ALWAYS draw indicators if item is dragged (regardless of current mode)
    // Draw red circle at original position
    ctx.fillStyle = "#8B0000";
    ctx.strokeStyle = "white";
    ctx.lineWidth = 1.5;
  ...
  loadImageCached(poi.logoUrl)
    .then((img) => {
      ...
      // ⭐ TIGHT FIT: Minimal padding (reduced from 12 to 4)
      const boxPadding = 4;
      ...
      // ⭐ THIN BORDER: Reduced from 3 to 1.5
      ctx.lineWidth = 1.5;
      ...
      // Draw logo centered in box (no extra margin)
      ctx.drawImage(
        img,
        x - logoWidth / 2,
        y - logoHeight / 2,
